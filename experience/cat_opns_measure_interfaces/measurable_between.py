...
class MeasurableBetween(MeasurableInContext):
    ...
    def angle_to(self, i_other_object: 'AnyObject', i_other_math_axis: list = None) -> tuple[float, float, float, float, float, float, float]:
    ...
    def angle_to_value_only(self, i_other_object: 'AnyObject', i_other_math_axis: list = None) -> float:
        return self.angle_to(i_other_object, i_other_math_axis)[0]

    def distance_min_to(self, i_other_object: 'AnyObject', i_other_math_axis: list = None) -> tuple[float, float, float, float, float, float, float]:
        i_other_math_axis =  i_other_math_axis if i_other_math_axis is not None else [None] * 12
        return self._get_multi([self.measurable_between, i_other_object._com, i_other_math_axis],
                               ("MeasurableBetween", "DistanceMinTo", "AnyObject", "Variant"),
                               ("Double", "Double", "Double", "Double", "Double", "Double", "Double"))
    
    def distance_min_to_value_only(self, i_other_object: 'AnyObject', i_other_math_axis: list = None) -> float:
        return self.distance_min_to(i_other_object, i_other_math_axis)[0]


    def distance_min_to_as(self, i_other_object: 'AnyObject', i_other_math_axis: list) -> tuple[float, float, float, float, float, float, float]:
        i_other_math_axis =  i_other_math_axis if i_other_math_axis is not None else [None] * 12
        return self._get_multi([self.measurable_between, i_other_object._com, i_other_math_axis],
    ...
    def set_computation_mode(self, i_computation_mode: CATMeasurableModeOfCalc) -> 'MeasurableBetween':
        self.measurable_between.SetComputationMode(int(i_computation_mode))
        return self
